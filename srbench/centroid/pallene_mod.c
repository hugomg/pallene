...
static Table * function_04(
    ...
    PALLENE_INT_FOR_LOOP_BEGIN(x6, 1, x2, 1)
    {
        x3 = 0.0;
        x4 = 0.0;
        PALLENE_INT_FOR_LOOP_BEGIN(x7, 1, x1, 1)
        {
            x9 = (lua_Number) x7;
            x8 = x9 * 3.1415000000000002;
            x3 = x3 + x8;
            x11 = (lua_Number) x7;
            x10 = x11 * 3.1415000000000002;
            x4 = x4 + x10;
        }
        PALLENE_INT_FOR_LOOP_END
    }
    PALLENE_INT_FOR_LOOP_END
    x12 = pallene_createtable(L, 2, 0);
    sethvalue(L, s2v(base + 0), x12);
    luaC_condGC(L, L->top = base + 1, (void)0);
    x14 = (lua_Number) x5;
    x13 = x3 / x14;
    {
        pallene_renormalize_array(L, x12, 1, 33);
        TValue *slot = &x12->array[1 - 1];
        setfltvalue(slot, x13);
    }
    x16 = (lua_Number) x5;
    x15 = x4 / x16;
    {
        pallene_renormalize_array(L, x12, 2, 33);
        TValue *slot = &x12->array[2 - 1];
        setfltvalue(slot, x15);
    }
    return x12;
}
...
int luaopen_srbench_centroid_pallene_mod(lua_State *L)
{
    luaL_checkversion(L);
