...
/* new srbench/objmandelbrot/pallene.pln:1 */
static Table * function_02(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* clone srbench/objmandelbrot/pallene.pln:5 */
static Table * function_03(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* conj srbench/objmandelbrot/pallene.pln:9 */
static Table * function_04(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* add srbench/objmandelbrot/pallene.pln:13 */
static Table * function_05(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* mul srbench/objmandelbrot/pallene.pln:17 */
static Table * function_06(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* norm2 srbench/objmandelbrot/pallene.pln:21 */
static lua_Number function_07(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
/* all srbench/objmandelbrot/pallene.pln:25 */
static void function_08(
    lua_State *L,
    Udata *G,
    StackValue *base,
    ...
    x11 = (lua_Number) x1;
    x9 = x10 / x11;
    PALLENE_INT_FOR_LOOP_BEGIN(x12, 1, x1, 1)
    {
        x16 = intop(-, x12, 1);
        x15 = (lua_Number) x16;
        x14 = x15 * x6;
        x13 = x2 + x14;
        PALLENE_INT_FOR_LOOP_BEGIN(x17, 1, x1, 1)
        {
            x21 = intop(-, x17, 1);
            x20 = (lua_Number) x21;
            x19 = x20 * x9;
            x18 = x4 + x19;
            x22 = function_02(L, G, base + 0, x13, x18);
            base = restorestack(L, base_offset);
            sethvalue(L, s2v(base + 0), x22);
            x23 = function_03(L, G, base + 1, x22);
            base = restorestack(L, base_offset);
            sethvalue(L, s2v(base + 1), x23);
            x24 = 0;
            while (1) {
                x25 = function_06(L, G, base + 2, x23, x23);
                base = restorestack(L, base_offset);
                sethvalue(L, s2v(base + 2), x25);
                x23 = function_05(L, G, base + 3, x25, x22);
                base = restorestack(L, base_offset);
                sethvalue(L, s2v(base + 1), x23);
                x24 = intop(+, x24, 1);
                x27 = function_07(L, G, base + 2, x23);
                base = restorestack(L, base_offset);
                x26 = (x27 > 4.0);
                if (!x26) {
                    x26 = (x24 > 255);
...
static int function_08_lua(lua_State *L)
{
    ...
    int nargs = lua_gettop(L);
    if (PALLENE_UNLIKELY(nargs != 1)) {
        pallene_runtime_arity_error(L, "all", 1, nargs);
    }
    ...
    if (PALLENE_UNLIKELY(!ttisinteger(s2v(base + 1)))) {
        pallene_runtime_tag_check_error(L,
            25, LUA_VNUMINT, rawtt(s2v(base + 1)),
            "argument '%s'", "N");
    }
    x1 = ivalue(s2v(base + 1));
    	
    function_08(L, G, L->top, x1);
    return 0;
}

int luaopen_srbench_objmandelbrot_pallene(lua_State *L)
{
    luaL_checkversion(L);
    ...
    lua_pushvalue(L, globals);
    lua_pushcclosure(L, function_08_lua, 1);
