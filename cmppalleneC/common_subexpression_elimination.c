...
/* cse cmppalleneC/common_subexpression_elimination.pln:1 */
static lua_Integer function_02(
    lua_State *L,
    Udata *G,
    StackValue *base,
...
static int function_02_lua(lua_State *L)
{
    ...
    if (PALLENE_UNLIKELY(nargs != 2)) {
        pallene_runtime_arity_error(L, "cse", 2, nargs);
    }
...
int luaopen_cmppalleneC_common_subexpression_elimination(lua_State *L)
{
    luaL_checkversion(L);
